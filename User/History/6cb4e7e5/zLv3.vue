        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form @submit.prevent="submitCustomer">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <button type="submit" class="btn btn-primary" @click="submitCustomer">Submit</button>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6">
                            <button type="submit" class="btn btn-primary" @click="updateCustomerOnSubmit">Update</button>
                        </div>
                    </div>
                </form>
            </div>
                        ...
                        <tbody>
                            <tr v-for="(customer, index) in customers" :key="index">
                                ...
                                <td>
                                    ...
                                    <button @click="updateCustomer(customer)" class="btn btn-primary">Update</button>
                                </td>
                            </tr>
                        </tbody>
                    </table>
...
export default {
    setup() {
        ...
        const deleteCustomer = async (customerId) => {
            try {
                // const response = await axios.delete(`http://127.0.0.1:8000/api/customer/customers/${customerId}`);
        ...
        const updateCustomer = async (customerData) => {
            ...
            customer.createdBy = customerData.created_by;
            customer.createdAt = customerData.createdAt || new Date().toISOString(); // Initialize if not provided
            customer.updatedAt = customerData.updatedAt || new Date().toISOString(); // Initialize if not provided
        };
        ...
        const updateCustomerOnSubmit = async () => {
            try {
                ...
                console.log(response.data);
                // Reset customer object after successful update
                Object.keys(customer).forEach(key => {
                    customer[key] = '';
                });
                await fetchCustomers();
