      ...
      <div class="card-body">
        <form @submit.prevent="submitSolution" class="form">
          ...
          <div class="row mt-3">
            ...
            <div class="col-md-6">
              <button type="submit" class="btn btn-primary" @click="updateSolutiononSubmit(solution.id)">Update Solution</button>
            </div>
          </div>
...
export default {
  setup() {
    ...
    const updateSolutiononSubmit =async (solutionId) => {
      const solutionToUpdate = solutions.value.find(solution => solution.id === solutionId);
        if (!solutionToUpdate) {
