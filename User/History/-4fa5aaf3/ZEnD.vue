      ...
      <div class="card-body">
        <form @submit.prevent="submitSolution" class="form">
          ...
          <div class="form-group">
            ...
            <input type="file" id="solution_image" @change="handleImageUpload" class="form-control" accept="image/*"
              required>
          </div>
          ...
          <div class="row mt-3">
            ...
            <div class="col-md-6">
              <button type="submit" class="btn btn-primary" @click="updateSolutiononSubmit(solution.id)">Update
                Solution</button>
            </div>
          </div>
...
export default {
  setup() {
    ...
    const updateSolutionForm = (solutionData) => {
      ...
      solution.createdAt = solutionData.createdAt ? new Date(solutionData.createdAt).toISOString().slice(0, 16) : '';
      solution.updatedAt = new Date().toISOString().slice(0, 16); //
      solution.entity = solutionData.entity;
    };
    const updateSolutiononSubmit = async (id) => {
      try {
        if (!solution.id) {
          console.error('solution  is undefined');
          return;
        }
        const response = await axios.put(`http://127.0.0.1:8000/api/solutions/${id}/update/`, solution);
        console.log(response.data);
        Object.keys(solution).forEach(key => {
          solution[key] = '';
        });
    ...
    return {
      ...
      updateSolutionForm,
      formatDate,
      updateSolutiononSubmit// Make formatDate function available to the template
    };
  }
};
</script>
