                    ...
                    <div class="row mt-3">
                        <div class="col-md-12 d-flex justify-content-between">
                            ...
                            <button type="button" class="btn btn-primary"
                                @click="updateAchievement(achievement)">Update</button>
                        </div>
                    </div>
                </form>
            </div>
...
export default {
    setup() {
        ...
        const updateAchievementForm = (selectedAchievement) => {
            ...
            achievement.created_at = new Date().toISOString().slice(0, 16);
            achievement.updated_at = new Date().toISOString().slice(0, 16);
            achievement.entity = selectedAchievement.entity;
            achievement.customer = selectedAchievement.customer;
        };
        const updateAchievement = async () => {
            try {
                const response = await axios.put(`http://127.0.0.1:8000/api/achievement/${achievement.id}/update`, achievement);
                console.log(response.data);
                fetchAchievements();
            } catch (error) {
