        ...
        <form @submit.prevent="submitEntity" class="form">
          ...
          <div class="row mt-3">
            <div class="col-md-12 d-flex justify-content-between">
              ...
              <button type="button" class="btn btn-primary" @click="updateEntity(entity.id)">Update</button>
            </div>
          </div>
        </form>
      </div>
...
export default {
  ...
  try {
    ...
    const response = await axios.put(`http://127.0.0.1:8000/api/entity/${entityData.id}/update/`, entity);
    console.log(response.data);
    Object.keys(entity).forEach(key => {
      entity[key] = '';
    });
